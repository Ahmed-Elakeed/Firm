...
public class OwnerDAO {
    ...
    public List<Owner> findAllOwners(){
        ...
        Owner owner;
        try {
            preparedStatement= DatabaseConfigurations.getConnection().prepareStatement("select * from owner");
    ...
    public Owner findOwnerByID(Integer ID) {
        Owner owner = null;
        try {
            preparedStatement = DatabaseConfigurations.getConnection().prepareStatement("select * from owner where owner_id=?");
    ...
    public void insertOwner(Owner owner) {
        try {
            preparedStatement = DatabaseConfigurations.getConnection().prepareStatement("insert into owner values (?,?)");
    ...
    public void deleteOwnerByID(Integer ID){
        try {
            preparedStatement = DatabaseConfigurations.getConnection().prepareStatement("delete from owner where owner_id=?");
            preparedStatement.setObject(1,ID);
            preparedStatement.executeUpdate();
        } catch (SQLException throwable) {
            throwable.printStackTrace();
...
//        try {
//            preparedStatement = DatabaseConfigurations.getConnection().prepareStatement("delete from owner where "+columnName+"=?");
...
//        try {
//            preparedStatement = DatabaseConfigurations.getConnection().prepareStatement("update owner set "+columnName+"=? where "+columnCheckName+"=?");
